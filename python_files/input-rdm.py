...
models = np.array([resnet34(pretrained=True)
...
NR_OF_SAMPLES = 10000 #num classes*num samples/class
SAVE_PATH = ROOT_PATH   
...
for model, model_name in zip(models, model_names):
    ...
    for layer_name, m in model.named_modules(): #name of layer (name), what happens in the layer (m) 
        ...
        if dots == 1: #different if statements for diff models/layers      
            ...
            handle.remove() #remve hook
            z = np.array(activations).shape[1]
            y = np.array(activations).shape[2]
            x = np.array(activations).shape[3]
            w = np.array(activations).shape[4]
            print('z,y,x,w->',z,y,x,w)
            flattened = np.array(activations).reshape(NR_OF_SAMPLES,z*y*x*w)
